#!/usr/bin/env python3
"""2021 国考行测两卷组装脚本（省级/市地级，无行政执法卷）。

输入（2021/xingce/_extract/）：
...
用法：python3 scripts/xingce/assemble_2021.py
"""
...
Y21 = DATA / "2021" / "xingce"
EX = Y21 / "_extract"

YEAR = 2021
EXAM_NAME = "2021年度国家公务员考试"
...
# offset: 常识0 / 言语0 / 数量0 / 判断+5 / 资料+5
SHENGJI_SHARED = {
    "常识判断": [2, 3, 4, 5, 6, 7, 8, 12, 13, 15, 16, 18, 20],
    "言语理解与表达": [22, 23, 24, 25, 26, 31, 32, 33, 34, 37, 38, 39, 40, 41, 42, 45, 47, 49, 50, 52, 53, 54, 55, 56, 59],
    "数量关系": [61, 62, 63, 64, 65, 66, 67, 68, 69],
    "判断推理": [72, 73, 74, 75, 76, 78, 79, 80, 82, 83, 84, 85, 86, 87, 88, 90, 92, 93, 94, 95, 96, 97, 98, 100, 101, 102, 103, 104, 105],
    "资料分析": [116, 117, 118, 119, 120, 121, 122, 123, 124, 125],
}
...
def build_paper(questions, modules, paper_type, total, material_pool, notes_extra=None):
    ...
    return {
        ...
        "notes": notes_extra or "2021 组装：省级135=市地共享86+省级独有49；答案待接入。",
        "media_index": {},
        "sections": sections,
    }
...
def assemble():
    ...
    merged = {
        ...
        "notes": "2021 两卷合并（shengji/shidi），无行政执法卷。",
    }
    ...
    diff_map = {
        "schema_version": 2,
        "exam_year": YEAR,
        "generated_by": "assemble_2021.py",
        "note": "省级卷共享题号=市地号+offset(判断/资料+5,其余+0)；2021无行政执法卷。",
        "mapping": {
            "shengji": {
                name: {
