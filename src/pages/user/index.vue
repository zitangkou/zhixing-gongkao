<template>
  <view class="page-user page-with-tabbar">
    <view class="profile-header">
      <view class="profile-main" @tap="go('/pages/user/profile')">
        <nut-avatar size="large" class="avatar">
          <image v-if="avatarUrl" class="avatar-img" :src="avatarUrl" mode="aspectFill" />
          <text v-else>{{ userStore.userInfo?.nickname?.slice(0, 1) || '学' }}</text>
        </nut-avatar>
        <view class="info">
          <text class="name">{{ userStore.userInfo?.nickname || '知行学员' }}</text>
    ...
    <view class="menu-group">
      <nut-cell title="今日清单" is-link @click="go('/pages/plan/today')">
        <template #icon>
          <view class="cell-icon"><CheckChecked :color="brandIcon" size="18" /></view>
        </template>
      </nut-cell>
      ...
      <nut-cell title="每日签到" is-link @click="go('/pages/user/signin')">
        <template #icon>
          <view class="cell-icon"><Clock :color="brandIcon" size="18" /></view>
        </template>
        <template #link>
      ...
      <nut-cell title="知行足迹" is-link @click="go('/pages/user/growth')">
        <template #icon>
          <view class="cell-icon"><Fabulous :color="brandIcon" size="18" /></view>
        </template>
      </nut-cell>
      ...
      <nut-cell title="刷题排行" is-link @click="go('/pages/user/rank')">
        <template #icon>
          <view class="cell-icon"><StarFill :color="brandIcon" size="18" /></view>
        </template>
      </nut-cell>
    </view>
    ...
    <view class="menu-group">
      ...
      <nut-cell title="知识框架" is-link @click="go('/pages/knowledge/index')">
        <template #icon>
          <view class="cell-icon"><Category :color="brandIcon" size="18" /></view>
        </template>
      </nut-cell>
      ...
      >
        ...
        <template #link>
          <text class="collapse-arrow" aria-hidden="true">{{ expanded.wrong ? '▾' : '▸' }}</text>
        </template>
      </nut-cell>
      <view v-if="expanded.wrong">
        <nut-cell title="文章错题" is-link @click="go('/pages/question/wrong')">
          <template #icon>
            <view class="cell-icon sub-icon"><Order :color="brandIcon" size="18" /></view>
          </template>
        </nut-cell>
        <nut-cell title="行测错题" is-link @click="go('/pages/question/manual-list')">
          <template #icon>
            <view class="cell-icon sub-icon"><Edit :color="brandIcon" size="18" /></view>
          </template>
        </nut-cell>
      </view>
    </view>
    ...
    <view class="menu-group">
      ...
      <nut-cell title="反馈建议" is-link @click="go('/pages/user/feedback')">
        <template #icon>
          <view class="cell-icon"><Message :color="brandIcon" size="18" /></view>
        </template>
      </nut-cell>
      <nut-cell title="退出登录" is-link @click="onLogout">
        <template #icon>
          <view class="cell-icon"><PoweroffCircleFill color="#999999" size="18" /></view>
        </template>
      </nut-cell>
    </view>
