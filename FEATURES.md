...
独立申论应用 `apps/shenlun-app` 已按原版迁移完整三刀解剖核心：骨架、规范词、金句、动词、句式五模块，保留分类/模板、动态增删、语音输入、分模块保存、历史恢复和删除；小题作答、自检与表达沉淀恢复为拆解后的独立页面。开采本、规范词库、阶梯训练也已恢复原版页面结构、字段、接口和交互；文章详情的全文选择、语料采集闭环仍按迁移基线继续补齐。
