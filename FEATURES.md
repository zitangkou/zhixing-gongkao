...
独立政治理论应用 `apps/theory-app` 已开始按原版拆回四段学习闭环：读前两道第一判断恢复为独立“读前定向”页，答题后的原文依据、错题筛选、逐题错因与完成状态恢复为独立“错因与依据”页；答题结果同时保存用户答案、错题 ID 和正确率，保证复盘页字段与原版一致。分节精读和通用刷题页仍按基线继续迁移。
