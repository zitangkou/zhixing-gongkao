...
| 品牌色 | 亮色 `#1E3A5F` / 暗色 `#3D5A7A`；`useBrandColor()` 供图标 props |
...
| 行为事件统计 | `activity_events` 已埋点写入，但无统计/可视化页面（M4：上岸卡片 / 能力雷达 / 里程碑） |
...
*最后更新：2026-08-13 · 基于 app.config.ts 52 个页面路由 + 23 个共享组件全量整理*
