...
### 2024 三卷答案+解析入库（覆盖率 100%）

**从 4 册解析册扫描件（77 页）提取 2024 年三卷答案与解析，经视觉提取 + 偏移映射 + 合并写入 papers。**

- **源文件**：`~/真题文档/2024/解析/` 下 4 册（240124/2425-/24-1/24-2），主答案册（240124+2425-）覆盖省级卷Q1-Q135，差异题册（24-1+24-2）覆盖市地差异题67题+执法差异题36题。
- **各册覆盖区间**：
  - 240124.pdf（24页，p78-101）：常识Q1-言语Q60-数量Q61-73
  - 2425-.pdf（19页，p102-120）：数量Q73-75→判断Q76-115→资料Q116-135
  - 24-1.pdf（24页，p121-144）：市地差异题①-67 + 执法差异题68-72
  - 24-2.pdf（10页，p144-153）：执法差异题续73-103
- **重大发现**：市地/执法卷现有125题**全部来自省级卷**（题号偏移：Q1-55直映、Q56-65偏移+5、Q66-125偏移+10）。差异题（市地67/执法36）在papers JSON中无对应题干，已完整提取但未参与merge，按硬约束不手改papers JSON补题。
- **映射方式**：新增 `2024/xingce/_extract/map_answers_2024.py`。省级直映主册Q1-Q135；市地/执法按偏移映射。
- **合并**：`scripts/merge_answers.py --year 2024 --answer-source answer_book_2026-09`，dry-run 0 rejected，正式写入。
- **覆盖率**：

| 卷 | 题量 | 已答 | 覆盖率 | rejected | pending(anchor) |
|---|---|---|---|---|---|
| 省级 | 135 | 135 | 100% | 0 | 111 |
| 市地 | 125 | 125 | 100% | 0 | 101 |
| 行政执法 | 125 | 125 | 100% | 0 | 101 |
| **合计** | **385** | **385** | **100%** | **0** | **313** |

- **anchor_pending**：313题标记。解析采用精炼摘要而非原文逐字转录，锚点比对得分低；题号映射已通过逐题题干内容验证，答案可信（同2025/2023情况）。
- **校验**：`validate_papers.py --year 2024`：**0 错误 / 37 警告**（警告均为预存media文件缺失，非本次合并引入）。
- **抽查**：省级16题、市地10题偏移映射，全部与解析册一致；answer_missing flag全部清除。
- **缺口登记**：市地/执法差异题共103题（市地67+执法36）已提取但papers JSON无对应题干，保存于 `_extract/raw_2024_shidi_diff.json` 和 `_extract/raw_2024_xzzf_diff.json`，待后续重建题干时使用。市地/执法题量各缺5题（言语模块）。
- **中间产物**：`2024/xingce/_extract/` 下 `raw_2024_main.json`、`raw_2024_shidi_diff.json`、`raw_2024_xzzf_diff.json`、`answers_{shengji,shidi,xzzf}.json`、`merge_report.json`、`unmatched_diff.json`、`map_answers_2024.py`、`2024_answer接入报告.md`。
- **同步更新**：`catalog.json`（2024 status→answers_ingested, answer_coverage→385/385）、`DATA_STATUS.md`（总览表+答案进度+2024详细说明）、`meta.json`（status+notes+answer_source）。

### 2023 三卷答案+解析入库（覆盖率 95.2%）

**从 4 册解析册扫描件（75 页）提取 2023 年三卷答案与解析，经 OCR + 映射 + 合并写入 papers。**

- **源文件**：`~/真题文档/2023/解析/` 下 4 册（23-1/23-22/23-3/23-4），三卷合一共用一套答案，主卷 130 题（市地/执法编号）+ 省级差异题 85 题 + 行政执法差异题 36 题。
- **提取方式**：macOS Vision 框架（Swift）OCR 批量识别 75 页 → `raw_2023.json`（250 条，含 module/anchor_stem/diff_type 标记）。解析册按模块分册，判断推理与资料分析题号重复，提取时按模块+题号双重定位。
- **映射方式**：新增 `scripts/map_answers_2023.py`。市地 Q1-130 直接映射；省级 shared(49) by from_number、province_only(86) 全局贪心内容匹配省级差异题；执法 shared_with_shidi(64) by from_number、shared(30) 匹配省级差异题、diff_book(36) 匹配行政执法差异题。内容匹配采用题干前缀子串 + 任意位置子串 + 滑动窗口相似度 + 4-gram 关键词命中率 + 2-gram 重叠五级策略。
- **合并**：`scripts/merge_answers.py --year 2023 --answer-source answer_book_2026-09`，dry-run 0 rejected，正式写入。
- **覆盖率**：
...
- **19 题未匹配原因**：图形推理 9 题（题干为通用模板"从所给的四个选项中…"无文本区分度，选项在图中）、选词填空/文章阅读 10 题（选项为短词或题干引用文章无独立关键词）。待后续人工逐题核对补全。
- **anchor_pending**：188 题标记。解析册解析以"完整解析："开头，引用题干预设结论或文献原文而非题干原句，脚本化锚点比对得分低；映射阶段已通过题号/内容匹配确保答案归属正确，该 flag 不代表答案错误（同 2025 情况）。
- **校验**：`validate_papers.py --year 2023`：**0 错误 / 28 警告**（警告均为预存图形推理 options=null + 同题干模板，与答案接入无关）。
- **抽查**：市地 Q1-25 共 25 题答案与解析册逐题比对，0 错误；省级 shared 49 题全部与市地 from_number 一致；执法 shared_with_shidi 64 题全部与市地一致。
- **中间产物**：`2023/xingce/_extract/` 下 `raw_2023.json`、`answers_{shidi,shengji,xzzf}.json`、`merge_report.json`、`mapping_unmatched.json`。
- **同步更新**：`catalog.json`（2023 status→answers_ingested, answer_coverage→376/395）、`DATA_STATUS.md`（总览表+答案进度+2023 详细说明）。
